...
static uint _sb_scan (sb_info_t * si, uint32 sba, void *regs, uint bus,
					   ...
					   char **vars, uint * varsz)
{
  ...
  if (BUSTYPE (si->sb.bustype) == SB_BUS &&
      ...
      (si->sb.chippkg != BCM4712LARGE_PKG_ID) && (si->sb.chiprev <= 3))
...
static uint BCMINITFN (_sb_coreidx) (sb_info_t * si, uint32 sba)
{
  uint i;
...
static uint32 BCMINITFN (_sb_coresba) (sb_info_t * si)
{
  uint32 sbaddr;
...
sb_corereg (sb_t * sbh, uint coreidx, uint regoff, uint mask, uint val)
{
  ...
  if (BUSTYPE (si->sb.bustype) == SB_BUS)
  ...
  else if (BUSTYPE (si->sb.bustype) == PCI_BUS)
    {
	  ...
	  ((si->sb.buscoretype == SB_PCIE) || (si->sb.buscorerev >= 13)))
	{
	  /* Chipc registers are mapped at 12KB */

	  fast = TRUE;
	  r =
	    (uint32 *) ((char *) si->curmap + PCI_16KB0_CCREGS_OFFSET +
			regoff);
	}
...
static uint
BCMINITFN (_sb_scan) (sb_info_t * si, uint32 sba, void *regs, uint bus,
		      uint32 sbba, uint numcores)
{
   ...
   */
  for (i = 0, next = si->numcores; i < numcores && next < SB_MAXCORES;
       i++, next++)
    {
...
static void BCMINITFN (sb_scan) (sb_info_t * si, void *regs, uint devid)
{
  uint origidx;
  uint32 origsba;
  uint i;
