...
robo_info_t *
bcm_robo_attach (sb_t * sbh, void *h, char *vars, miird_f miird,
		 miiwr_f miiwr)
{
  robo_info_t *robo;
  uint32 reset, idx;
  ...
  if ((robo->devid == DEVID5395) ||
      (robo->devid == DEVID5397) || (robo->devid == DEVID5398))
    {
      uint8 srst_ctrl;
  ...
  if (robo->devid == DEVID5398)
    {
      ...
      for (reg_offset = REG_CTRL_PORT6; reg_offset <= REG_CTRL_PORT7;
	   reg_offset++)
	{
...
bcm_robo_config_vlan (robo_info_t * robo, uint8 * mac_addr)
{
  ...
  int pdescsz;
  uint16 vid;
  uint8 arl_entry[8] = { 0 }, arl_entry1[8] =
  {
  0};
  ...
  robo->ops->write_reg (robo, PAGE_VLAN, REG_VLAN_CTRL0, &val8,
			sizeof (val8));
  ...
  robo->ops->write_reg (robo, PAGE_VLAN, REG_VLAN_CTRL1, &val8,
			sizeof (val8));
  ...
  if (robo->devid == DEVID5325)
    {
      ...
      robo->ops->write_reg (robo, PAGE_VTBL, REG_VTBL_VID_E1, arl_entry1, 1);
      ...
      robo->ops->write_reg (robo, PAGE_VLAN, REG_VLAN_CTRL4, &val8,
			    sizeof (val8));

      /* VLAN Control 5 Register (Page 0x34, Address 5) */
      val8 = (1 << 3);		/* drop frame when miss V table */
      robo->ops->write_reg (robo, PAGE_VLAN, REG_VLAN_CTRL5, &val8,
			    sizeof (val8));
...
#define	FL	FLAG_UNTAG
      ...
      else
	{
	  ...
	  val8 = ((1 << 7) |	/* start command */
		  0);		/* write */
	  robo->ops->write_reg (robo, PAGE_VTBL, vtbla, &val8, sizeof (val8));
	}
    }
  ...
  if (robo->devid == DEVID5325)
    {
      ...
      val32 = ((0 << 0) |	/* 0 -> 0 */
	       ...
	       (6 << 18) |	/* 6 -> 6 */
	       (7 << 21));	/* 7 -> 7 */
      robo->ops->write_reg (robo, PAGE_VLAN, REG_VLAN_PMAP, &val32,
			    sizeof (val32));
    }
  ...
  if (!(val8 & (1 << 1)))
    {
      ...
      /* Enable forwarding */
      val8 |= (1 << 1);
      robo->ops->write_reg (robo, PAGE_CTRL, REG_CTRL_MODE, &val8,
			    sizeof (val8));

      /* Read back */
      robo->ops->read_reg (robo, PAGE_CTRL, REG_CTRL_MODE, &val8,
			   sizeof (val8));
      if (!(val8 & (1 << 1)))
	{
      ...
      val8 = 0;
      max_port_ind =
	((robo->devid == DEVID5398) ? REG_CTRL_PORT7 : REG_CTRL_PORT4);
...
robo_dump_regs (robo_info_t * robo, struct bcmstrbuf *b)
{
  ...
  if (robo->devid == DEVID5325)
    {
      ...
      robo->ops->read_reg (robo, PAGE_VLAN, REG_VLAN_CTRL5, &val8,
			   sizeof (val8));
      bcm_bprintf (b, "(0x34,0x05)VLAN control 5 register: 0x%02x\n", val8);

      robo->ops->read_reg (robo, PAGE_VLAN, REG_VLAN_PMAP, &val32,
			   sizeof (val32));
      ...
      for (i = 0; i <= VLAN_MAXVID; i++)
	{
	  ...
	  robo->ops->write_reg (robo, PAGE_VLAN, REG_VLAN_ACCESS, &val16,
				sizeof (val16));
	  robo->ops->read_reg (robo, PAGE_VLAN, REG_VLAN_READ, &val32,
			       sizeof (val32));
	  bcm_bprintf (b,
  ...
  for (i = 0; i < pdescsz; i++)
    {
      robo->ops->read_reg (robo, PAGE_VLAN, pdesc[i].ptagr, &val16,
			   sizeof (val16));
